...
nz = 128  # dimensionality of the learned embedding
...
for sample_img, sample_label in mnist_data_loader:
    ...
    for i, (image, embedding) in enumerate(zip(sample_img, enc)):
        print(i + index, torch.squeeze(image).shape, embedding.shape)
        im_data[i + index, :,:] = torch.squeeze(image).numpy()
        em_data[i + index, :] = embedding.cpu().detach().numpy()

    index += i

np.savez_compressed('./data/embeddings', images=im_data, embeddings=em_data)
